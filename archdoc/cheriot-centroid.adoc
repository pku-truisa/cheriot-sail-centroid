...
规整块标记指针（Block-Type, B-Type）：所有对象是2的幂次方大小和对齐。用户程序可以通过CSetBound和CSetBoundImm指令设定，需要符合单调性原则。该类型标记指针与cheriot定义相同。
...
|26 |MRW |MDESCRC | Machine Descriptor Capability Register, 通用描述符寄存器1
|27 |MRW |MDDLBC  | Machine Data Descriptor Lookaside Buffer Capbility，通用描述符寄存器2
